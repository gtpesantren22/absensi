...
<head>
    ...
    <title>Jurnal Mengajar - Portal Guru</title>
...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 min-h-screen pb-28 transition-colors duration-200">
    ...
    <div class="bg-primary-600 dark:bg-slate-900 px-4 py-4 shadow-lg sticky top-0 z-50 flex items-center justify-between text-white">
        <div class="flex items-center space-x-3">
            ...
            <h1 class="text-lg font-bold tracking-wide">Jurnal Mengajar</h1>
        </div>
    ...
    <div class="px-4 py-5 space-y-6">
        ...
        <div class="flex justify-end">
            <button onclick="window.location.href='<?= base_url('mengajar/mengajar_add') ?>'" 
                    class="w-full py-3 px-4 rounded-2xl bg-primary-600 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-md hover:bg-primary-700 transition active:scale-95">
    ...
    <script>
        ...
        function loadData() {
            ...
            fetch(`<?= site_url('mengajar/mengajarData') ?>?${params}`)
                .then(res => res.json())
                .then(res => {
                ...
                .catch(err => {
                    document.getElementById('tableBody').innerHTML = `
                        <tr>
                            <td colspan="3" class="py-8 px-4 text-center text-rose-500 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm">
                                <i class="fas fa-exclamation-circle text-2xl mb-2"></i>
                                <p class="text-sm font-bold">Gagal memuat data jurnal mengajar.</p>
                            </td>
                        </tr>
                    `;
                });
        ...
        function renderTable(data) {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="3" class="py-8 px-4 text-center">
                            <div class="flex flex-col items-center justify-center text-slate-450 dark:text-slate-500">
                                <i class="fas fa-calendar-times text-2xl mb-2 text-slate-400"></i>
                                <p class="text-sm font-bold">Tidak ada data jurnal mengajar.</p>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            data.forEach(row => {
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                    <td class="py-3.5 px-4 font-extrabold text-slate-850 dark:text-white">
                        ${tanggalIndo(row.tanggal, true)}
                    </td>
                    <td class="py-3.5 px-4 font-extrabold text-slate-700 dark:text-slate-350 text-center">
                    ...
                    <td class="py-3.5 px-4 text-center">
                        <div class="flex items-center justify-center gap-1.5">
                            <button onclick="window.location.href='<?php echo base_url() ?>mengajar/mengajar_add/${row.id}'" 
                            ...
                            ${deleteBtn}
                            <button onclick="window.open('<?php echo base_url() ?>mengajar/screenMengajarGuru/${row.tanggal}','_blank')" 
        ...
        $(document).on('click', '.tombol-hapus', function(e) {
            ...
            Swal.fire({
                title: 'Hapus Jurnal?',
                text: 'Data jurnal mengajar guru tanggal ini akan dihapus permanen.',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.post(base_url + 'kehadiranguru/hapusKehadiran', { id })
                        .done(() => {
                            Swal.fire({
                                icon: 'success',
                                title: 'Terhapus!',
                                text: 'Jurnal mengajar berhasil dihapus.',
                                timer: 1500,
                        ...
                        .fail(() => {
                            Swal.fire({
                                icon: 'error',
                                title: 'Gagal',
                                text: 'Tidak dapat menghapus data jurnal.'
                            });
                        });
                }
            });
