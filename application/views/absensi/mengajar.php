...
<div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
    <div>
        <h2 class="text-2xl font-bold">Data Absensi Mengajar Guru</h2>
        <p class="text-gray-600 dark:text-gray-400">Halaman kelola data absensi mengajar guru</p>
    </div>

    <div class="flex flex-wrap items-center gap-2 mt-4 md:mt-0">

        <button onclick="window.location.href='<?= base_url('mengajar/mengajar_add') ?>'" class="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg font-medium flex items-center">
    ...
    function renderTable(data) {
        ...
        data.forEach(row => {
            ...
            <tr class="border-b">
                ...
                <td class="p-2">
                    <button onclick="window.location.href='<?php echo base_url() ?>mengajar/mengajar_add/${row.id}'" class="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                    <button data-id="${row.id}" class="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600 tombol-hapus">Hapus</button>
                    <button onclick="window.open('<?php echo base_url() ?>mengajar/screenMengajarGuru/${row.tanggal}','_blank')" class="px-2 py-1 bg-green-500 text-white rounded hover:bg-green-600"><i class="fas fa-download"></i></button>
                </td>
            </tr>
        `;
        });
